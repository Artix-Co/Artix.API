...
public static class DependencyInjection
{
    public static void AddRedis(this IServiceCollection services)
    {
        ...
        services.AddSingleton<ICacheService<RecentMuseumDto>>(sp =>
            new RedisCacheService<RecentMuseumDto>(sp.GetRequiredService<IConnectionMultiplexer>(),
                "recent:museums:user"));
        services.AddSingleton<ICacheService<RecentObjectDto>>(sp =>
            new RedisCacheService<RecentObjectDto>(sp.GetRequiredService<IConnectionMultiplexer>(),
                "recent:objects:user"));
    }
}
