...
public class UploadController : ClientBaseController
{
    ...
    {
        var s = await this._service.InitiateAsync(fileName, totalSize, chunkSize);
    ...
    public async Task<IActionResult> UploadChunk([FromRoute] Guid uploadId, [FromRoute] int index)
    {
        ...
        await this._service.MarkChunkReceivedAsync(uploadId, index);
        return this.Ok();
    }
    ...
    {
        await this._service.MergeChunksAsync(uploadId);
        return this.Ok();
    }
    ...
    {
        var s = await this._service.GetStatusAsync(uploadId);
        if (s == null) return this.NotFound();
        var received = s.ReceivedChunks.Keys.OrderBy(k => k).ToArray();
        return this.Ok(new { s.Id, s.FileName, s.TotalSize, s.ChunkSize, s.TotalChunks, received, s.Completed });
    }
}
